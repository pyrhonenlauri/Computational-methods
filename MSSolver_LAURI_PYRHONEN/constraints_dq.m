...
for tj=mbs.joints.translational
    ...
    A1 = rot(q1(3));
    A2 = rot(q2(3));
    Cq(c_idx + (1:2), q1_idx) = [(tj.s2P-tj.s2Q)'*A2'*eye(2), (tj.s2P-tj.s2Q)'*A2'*Omega * A1 * tj.s1O;
                                  zeros(1,2),1];
    Cq(c_idx + (1:2), q2_idx) = -[(tj.s2P-tj.s2Q)'*A2'*eye(2), -(tj.s2P-tj.s2Q)'*(Omega*A2)'*(r1+A1*tj.s1O-r2);
                                  zeros(1,2),1];
    c_idx = c_idx + 2;
end
